...
func cleanHeader(input string) (output string) {
	...
	source := regexp.MustCompile(`# Source: /tmp/ww4test-[0-9]*/`)

	output = input
	output = host.ReplaceAllString(output, "# Host:   REMOVED")
	output = time.ReplaceAllString(output, "# Time:   REMOVED")
	output = source.ReplaceAllString(output, "# Source: /REMOVED/")
	return output
}

// overlays/wwinit/rootfs/etc/warewulf/warewulf.conf.ww
// overlays/wwinit/rootfs/warewulf/config.ww
// overlays/wwinit/rootfs/warewulf/init.d/80-wwclient.ww
...
# Host:   REMOVED
# Time:   REMOVED
# Source: /REMOVED/var/lib/warewulf/overlays/ifcfg/rootfs/etc/sysconfig/network-scripts/ifcfg.ww
TYPE=ethernet
DEVICE=wwnet0
NAME=default
BOOTPROTO=static
