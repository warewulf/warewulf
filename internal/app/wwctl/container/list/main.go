...
func CobraRunE(vars *variables) func(cmd *cobra.Command, args []string) (err error) {
	return func(cmd *cobra.Command, args []string) (err error) {
		if vars.size || vars.full || vars.kernel {
			...
			if vars.full {
				ph := helper.NewPrintHelper([]string{"CONTAINER NAME", "NODES", "KERNEL VERSION", "CREATION TIME", "MODIFICATION TIME", "SIZE", "IMAGE", "COMPRESSED"})
				for i := 0; i < len(containerInfo); i++ {
					...
					ph.Append([]string{
						...
						util.ByteToString(int64(containerInfo[i].ImgSize)),
						util.ByteToString(int64(containerInfo[i].ImgSizeComp)),
					})
				}
				ph.Render()
			} else if vars.kernel {
				...
				ph := helper.NewPrintHelper([]string{"CONTAINER NAME", "NODES", "SIZE", "IMAGE", "COMPRESSED"})
				for i := 0; i < len(containerInfo); i++ {
					ph.Append([]string{
						containerInfo[i].Name,
						strconv.FormatUint(uint64(containerInfo[i].NodeCount), 10),
