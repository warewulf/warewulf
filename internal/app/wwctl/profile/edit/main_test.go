...
func Test_Profile_Edit(t *testing.T) {
	env := testenv.New(t)
	defer env.RemoveAll()
...
# Keep the header up to and including the separator, then append modified profiles with comments
sed -i '/# DO NOT EDIT ABOVE THIS LINE, CHANGES WILL BE LOST/q' "$1"
cat << 'EOF' >> "$1"
default:
  # This is a new comment from the editor for the default profile
  comment: edited profile comment
EOF
`
	...
	// 3. Run the profile edit command with --yes
	baseCmd := GetCommand()
	baseCmd.SetArgs([]string{"--yes", "default"})
	...
	assert.True(t, strings.Contains(content, "comment: edited profile comment"))
	assert.True(t, strings.Contains(content, "# This is a new comment from the editor for the default profile"))
	// Node comments should be preserved
	assert.True(t, strings.Contains(content, "# Original comment for node1"))
}
