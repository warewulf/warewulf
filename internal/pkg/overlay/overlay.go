...
// before deletion.
func (overlay Overlay) DeleteFile(filePath string, force, clenaup bool) error {
	if filePath == "" {
		if force {
		...
		wwlog.Info("Deleted file %s from overlay %s", filePath, overlay.Name())

		if clenaup {
			// clenaup the empty parents
			i := path.Dir(fullPath)
