...
VARLIST += WWCONFIGDIR WWPROVISIONDIR WWOVERLAYDIR WWCHROOTDIR WWTFTPDIR WWDOCDIR WWBASHCOMPDIR
...
WWBASHCOMPDIR ?= $(BASHCOMPDIR)/$(WAREWULF)
...
files: all
	...
	install -d -m 0755 $(DESTDIR)$(WWCONFIGDIR)
	install -d -m 0755 $(DESTDIR)$(WWCONFIGDIR)/ipxe
	install -d -m 0755 $(DESTDIR)$(WWTFTPDIR)/ipxe/
	install -d -m 0755 $(DESTDIR)$(WWBASHCOMPDIR)
	install -d -m 0755 $(DESTDIR)$(MANDIR)/man1
	install -d -m 0755 $(DESTDIR)$(WWDOCDIR)
	install -d -m 0755 $(DESTDIR)$(FIREWALLDDIR)
	install -d -m 0755 $(DESTDIR)$(SYSTEMDDIR)
	test -f $(DESTDIR)$(WWCONFIG)/warewulf.conf || install -m 644 etc/warewulf.conf $(DESTDIR)$(WWCONFIG)
	test -f $(DESTDIR)$(WWCONFIG)/hosts.tmpl || install -m 644 etc/hosts.tmpl $(DESTDIR)$(WWCONFIG)
	test -f $(DESTDIR)$(WWCONFIG)/nodes.conf || install -m 644 etc/nodes.conf $(DESTDIR)$(WWCONFIG)
	...
	install -m 0755 wwctl $(DESTDIR)$(WWBINDIR)
	...
	cp bash_completion.d/warewulf $(DESTDIR)$(WWBASHCOMPDIR)
...
clean:
	...
	rm -f config
	rm -f Defaults.mk
	rm -f internal/pkg/warewulfconf/buildconfig.go
	cp internal/pkg/warewulfconf/buildconfig.{stub,go}
