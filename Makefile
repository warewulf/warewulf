...
# Server-only overlays, installed 0750. "hosts" is excluded since it is
# also assigned to nodes.
SERVEROVERLAYS = dhcpd dnsmasq nfsd ssh.wwctl tftproot
...
install: build docs ## Install Warewulf from source
	...
	for o in $(SERVEROVERLAYS); do chmod 0750 $(DESTDIR)$(DATADIR)/warewulf/overlays/$$o/rootfs; done
