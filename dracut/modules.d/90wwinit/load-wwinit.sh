...
if [ -z ${wwinit_persistent} ] ; then 
	...
	(curl --retry 60 --retry-delay 1 --silent ${localport}  -L "http://${wwinit_ip}:${wwinit_port}/overlay/persistent/ignition.json?node=${wwinit_node}" | jq '.storage.filesystems |= map(select(.device=="/dev/disk/by-partlabel/rootfs").path="/")' > /run/ignition.json) || die "warewulf: failed to fetch ignition configuration"
	setup_disks
...
for archive in  $archives ; do
	...
	(curl --retry 60 --retry-delay 1 --silent ${localport} -L "${archive}" | gzip -d | cpio --quiet -u -im --directory="${NEWROOT}") || die "warewulf: Unable to load ${archive}"
    fi
done
if [ ! -z ${wwinit_persistent} ] ; then
	...
	(curl --retry 60 --retry-delay 1 --silent ${localport}  -L "http://${wwinit_ip}:${wwinit_port}/overlay/persistent/fstab.ww?node=${wwinit_node}" >> ${NEWROOT}/etc/fstab ) || die "warewulf: failed to write correct fstab"
fi
