...
Each partition has the switches ``should_exist`` and ``wipe_partition_entry`` which control the partition creation process. When omitting a partition number the `wipe_partition_entry` should be true, as this allows ignition to clobber the existing
partition.
...
Unless you specified the `--fswipe` flag for a filesystem, `ignition` will try to 
reuse existing file system. For empty disks this means that the desired configuration
...
* wipe the existing data with the means of tolls like `wipefs` or `dd` [#]
* set `--fswipe` flag and remove it after one reboot, if you want to keep
