...
- **/warewulf/wwinit.d/:** executed in the initial root final system before the
  ...
  the boot, and the remaining scripts in the directory are not run. A script
  must therefore exit with the status of the work it did: end it with an
  explicit ``exit 0``, so that a message which could not be written is never
  mistaken for a failed provisioning step.
